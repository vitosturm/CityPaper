...
import type { Newspaper, NewsArticle, Activity } from "@/types/newspaper";
    ...
    <main className="min-h-screen bg-amber-50 font-serif">
      ...
      {newspaper && !loading && (
        <section className="mx-auto max-w-4xl px-6 pb-16">
          ...
          {/* Activities */}
          {newspaper.activities?.length > 0 && (
            <div className="mt-6">
              <p className="mb-4 border-b-2 border-black pb-2 text-lg font-black uppercase tracking-widest">
                ...
                {newspaper.activities.map((activity: Activity, idx: number) => (
                  <div key={activity.name} className="border-2 border-black bg-white overflow-hidden">
                    ...
                    <div className="p-3">
                      ...
                      <p className="mt-1 font-black leading-tight">{activity.name}</p>
                      <p className="mt-1 text-xs leading-relaxed text-gray-600">{activity.description}</p>
                    </div>
                  </div>
                ))}
              </div>
